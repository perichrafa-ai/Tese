...
# -*- coding: utf-8 -*-
"""
master.py — Pipeline limpo para gerar apenas as Figuras 3.1–3.7 e H3-Logit da tese
------------------------------------------------------------------------------------
Este script foi refatorado para gerar SOMENTE as figuras que de fato aparecem na tese.

FIGURAS GERADAS (lista branca — 9 arquivos):
  1. fig_3_1_reeleicao_por_regiao
  2. fig_3_2_distribuicao_delta_vantagem
  3. fig_3_3_precocidade_npis
  4. fig_3_4_infraestrutura
  5. fig_3_5_estresse_saude
  6. fig_3_6_ifdm_variacoes
  7. fig_3_7_coeficientes_ols_completo (gera também: coeficientes_ols_completo.png)
  8. *__H3_logit_lnpop.png (Prob. prevista vs ln(Pop), Gestão ±1dp)
  9. *__H3_logit_lnpop_sev.png (Prob. prevista vs ln(Pop), Severidade ±1dp)

Como rodar (PowerShell, 1 linha):
  py .\master.py --csv ".\Base_VALIDADA_E_PRONTA.csv" --outdir ".\output" --encoding latin-1 --sep ";" --decimal ","
...
VALIDAÇÃO AUTOMÁTICA:
  Ao final da execução, o script valida que APENAS as 9 figuras-alvo foram geradas.
  Se alguma figura fora da lista branca for detectada, um RuntimeError será lançado.
"""
...
def write_table(df: pd.DataFrame, outdir: Path, nome: str):
    """
    Salva SOMENTE tabelas usadas na tese (whitelist). Tudo fora disso é ignorado.
    """
...
def find_ifdm_delta_cols(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str]]:
    """
    Retorna (col_emprego_renda, col_saude) para a Figura 3.6.
    Heurística robusta:
      - Normaliza acentos e trata símbolo 'Δ' como 'delta'
      - Aceita pistas de ano '2016-2020' ou '2020-2016'
      - Exige ('ifdm'|'firjan') + (emprego|renda) ou (saude|saúde) + (varia|delta|ano-range)
    """
    import unicodedata
...
def build_parte2_outputs(df: pd.DataFrame, outdir: Path):
    """Gera as saídas da parte 2 (interações)"""
...
def build_parte3_outputs(df: pd.DataFrame, outdir: Path):
    """Gera as saídas da parte 3 (H3/H4)"""
...
def validate_figures_whitelist(outdir: Path):
    """
    Valida que APENAS as 9 figuras-alvo da tese foram geradas.
    Lança RuntimeError se detectar figuras fora da lista branca.
    """
    logger.info('=' * 60)
...
def main():
    ...
    if args.novos == 'on':
        ...
        try:
            ...
            logger.error(f'Erro nas novas análises: {e}')
            import traceback
...
def run_novas_analises(df: pd.DataFrame, csv_path: Path, outdir: Path):
    """
    Executa análises adicionais conforme especificação.
    Todas as saídas vão para outdir/novos/ com prefixo do nome do CSV.
    """
    import json
    import datetime
    from pathlib import Path
    ...
    def save_artifact(name: str, content: str = None, df_: pd.DataFrame = None, fig_=None, json_obj=None):
        """Salva artefato e registra no manifesto"""
    ...
    # ===== 4.6 Sensibilidades =====
    logger.info("=== 4.6: Sensibilidades ===")

    # === 4.6.1: NEC → HHI ===
    logger.info("  - Sens 1/7: NEC → HHI")
    sens_nec_hhi = {'status': 'não executado', 'razao': 'falta coluna HHI ou votos'}

    # Verificar se temos votos para calcular HHI
    # HHI = sum(share_i^2) onde share_i é proporção de votos do candidato i
    # Se não temos, marcamos como pendente
    if 'HHI' in df_work.columns or 'hhi' in df_work.columns:
        hhi_col = 'HHI' if 'HHI' in df_work.columns else 'hhi'
        if 'delta_vantagem' in df_work.columns and hhi_col in df_work.columns:
            # Re-estimar modelo substituindo NEC por HHI
            df_hhi = df_work[['delta_vantagem', hhi_col, 'gestao_index', 'ln_pop',
                             'ifdm_geral', 'abstencao_var', 'precocidade']].dropna()
            if len(df_hhi) > 20:
                X_hhi = df_hhi[[hhi_col, 'gestao_index', 'ln_pop', 'ifdm_geral',
                               'abstencao_var', 'precocidade']]
                X_hhi = sm.add_constant(X_hhi)
                y_hhi = df_hhi['delta_vantagem']

                mod_hhi = sm.OLS(y_hhi, X_hhi).fit(cov_type='HC1')
                sens_nec_hhi = {
                    'status': 'executado',
                    'N': len(df_hhi),
                    'coef': mod_hhi.params.to_dict(),
                    'pvalues': mod_hhi.pvalues.to_dict(),
                    'AIC': float(mod_hhi.aic),
                    'nota': f'HHI coef={mod_hhi.params[hhi_col]:.4f}, p={mod_hhi.pvalues[hhi_col]:.4f}'
                }
    else:
        sens_nec_hhi['nota'] = 'HHI não disponível; requer cálculo a partir de votos individuais'

    save_artifact('Sens_NEC_HHI.json', json_obj=sens_nec_hhi)

    # === 4.6.2: NEC → n_candidatos ===
    logger.info("  - Sens 2/7: NEC → n_candidatos")
    sens_ncand = {'status': 'não executado', 'razao': 'falta n_candidatos'}

    # Buscar coluna com número de candidatos
    ncand_col = None
    for col in df_work.columns:
        if 'candidato' in col.lower() and 'numero' in col.lower():
            ncand_col = col
            break

    if ncand_col and 'delta_vantagem' in df_work.columns:
        df_ncand = df_work[['delta_vantagem', ncand_col, 'gestao_index', 'ln_pop',
                           'ifdm_geral', 'abstencao_var', 'precocidade']].dropna()
        if len(df_ncand) > 20:
            X_ncand = df_ncand[[ncand_col, 'gestao_index', 'ln_pop', 'ifdm_geral',
                               'abstencao_var', 'precocidade']]
            X_ncand = sm.add_constant(X_ncand)
            y_ncand = df_ncand['delta_vantagem']

            mod_ncand = sm.OLS(y_ncand, X_ncand).fit(cov_type='HC1')
            sens_ncand = {
                'status': 'executado',
                'N': len(df_ncand),
                'coef': mod_ncand.params.to_dict(),
                'pvalues': mod_ncand.pvalues.to_dict(),
                'AIC': float(mod_ncand.aic),
                'nota': f'n_cand coef={mod_ncand.params[ncand_col]:.4f}, p={mod_ncand.pvalues[ncand_col]:.4f}'
            }
    else:
        sens_ncand['nota'] = 'Coluna n_candidatos não encontrada'

    save_artifact('Sens_NEC_ncand.json', json_obj=sens_ncand)

    # === 4.6.3: Forma populacional (dummies por quartis) ===
    logger.info("  - Sens 3/7: Forma populacional")
    sens_formapop = {'status': 'não executado'}

    if 'ln_pop' in df_work.columns and 'delta_vantagem' in df_work.columns:
        df_fp = df_work[['delta_vantagem', 'NEC', 'gestao_index', 'ln_pop',
                        'ifdm_geral', 'abstencao_var', 'precocidade']].dropna()
        if len(df_fp) > 20:
            # Criar quartis de ln_pop
            df_fp['pop_q'] = pd.qcut(df_fp['ln_pop'], q=4, labels=False, duplicates='drop')
            # Dummies (omitir primeiro quartil)
            df_fp['pop_q2'] = (df_fp['pop_q'] == 1).astype(int)
            df_fp['pop_q3'] = (df_fp['pop_q'] == 2).astype(int)
            df_fp['pop_q4'] = (df_fp['pop_q'] == 3).astype(int)

            X_fp = df_fp[['NEC', 'gestao_index', 'pop_q2', 'pop_q3', 'pop_q4',
                         'ifdm_geral', 'abstencao_var', 'precocidade']]
            X_fp = sm.add_constant(X_fp)
            y_fp = df_fp['delta_vantagem']

            mod_fp = sm.OLS(y_fp, X_fp).fit(cov_type='HC1')
            sens_formapop = {
                'status': 'executado',
                'N': len(df_fp),
                'coef': mod_fp.params.to_dict(),
                'pvalues': mod_fp.pvalues.to_dict(),
                'AIC': float(mod_fp.aic),
                'nota': 'Quartis de população (ref=Q1): Q2, Q3, Q4'
            }

    save_artifact('Sens_FormaPop.json', json_obj=sens_formapop)

    # === 4.6.4: Sem capitais ===
    logger.info("  - Sens 4/7: Sem capitais")
    sens_semcap = {'status': 'não executado'}
    ...
    # Verificar se temos coluna de município
    mun_col = None
    for col in df_work.columns:
    ...
    if mun_col and 'delta_vantagem' in df_work.columns:
        df_sc = df_work[~df_work[mun_col].isin(capitais)].copy()
        df_sc = df_sc[['delta_vantagem', 'NEC', 'gestao_index', 'ln_pop',
                      'ifdm_geral', 'abstencao_var', 'precocidade']].dropna()

        if len(df_sc) > 20:
            X_sc = df_sc[['NEC', 'gestao_index', 'ln_pop', 'ifdm_geral',
                         'abstencao_var', 'precocidade']]
            X_sc = sm.add_constant(X_sc)
            y_sc = df_sc['delta_vantagem']

            mod_sc = sm.OLS(y_sc, X_sc).fit(cov_type='HC1')
            sens_semcap = {
                'status': 'executado',
                'N': len(df_sc),
                'N_excluidos': len(df_work) - len(df_sc),
                'coef': mod_sc.params.to_dict(),
                'pvalues': mod_sc.pvalues.to_dict(),
                'AIC': float(mod_sc.aic),
                'nota': f'Excluídas {len(df_work) - len(df_sc)} capitais'
            }
    else:
        sens_semcap['nota'] = 'Coluna de município não encontrada'

    save_artifact('Sens_SemCapitais.json', json_obj=sens_semcap)

    # === 4.6.5: Leave-One-UF-Out ===
    logger.info("  - Sens 5/7: Leave-One-UF-Out")
    sens_louo = {'status': 'não executado'}

    if 'UF' in df_work.columns and 'delta_vantagem' in df_work.columns:
        df_louo_base = df_work[['delta_vantagem', 'NEC', 'gestao_index', 'ln_pop',
                                'ifdm_geral', 'abstencao_var', 'precocidade', 'UF']].dropna()

        if len(df_louo_base) > 20:
            ufs = df_louo_base['UF'].unique()
            louo_results = {}

            for uf in ufs:
                df_excl = df_louo_base[df_louo_base['UF'] != uf].copy()
                if len(df_excl) > 15:
                    X_excl = df_excl[['NEC', 'gestao_index', 'ln_pop', 'ifdm_geral',
                                     'abstencao_var', 'precocidade']]
                    X_excl = sm.add_constant(X_excl)
                    y_excl = df_excl['delta_vantagem']

                    try:
                        mod_excl = sm.OLS(y_excl, X_excl).fit(cov_type='HC1')
                        louo_results[uf] = {
                            'N': len(df_excl),
                            'coef_NEC': float(mod_excl.params['NEC']),
                            'pval_NEC': float(mod_excl.pvalues['NEC']),
                            'AIC': float(mod_excl.aic)
                        }
                    except:
                        louo_results[uf] = {'erro': 'modelo não convergiu'}

            # Calcular estatísticas resumo
            nec_coefs = [v['coef_NEC'] for v in louo_results.values() if 'coef_NEC' in v]
            sens_louo = {
                'status': 'executado',
                'N_UFs': len(ufs),
                'resultados_por_UF': louo_results,
                'resumo_NEC': {
                    'min': float(np.min(nec_coefs)) if nec_coefs else None,
                    'max': float(np.max(nec_coefs)) if nec_coefs else None,
                    'mean': float(np.mean(nec_coefs)) if nec_coefs else None,
                    'std': float(np.std(nec_coefs)) if nec_coefs else None
                },
                'nota': f'Coef NEC varia de {np.min(nec_coefs):.3f} a {np.max(nec_coefs):.3f}' if nec_coefs else 'sem resultados'
            }

    save_artifact('Sens_LOUO.json', json_obj=sens_louo)

    # === 4.6.6: Winsorização ===
    logger.info("  - Sens 6/7: Winsorização")
    sens_winsor = {'status': 'não executado'}

    if 'delta_vantagem' in df_work.columns:
        df_win = df_work[['delta_vantagem', 'NEC', 'gestao_index', 'ln_pop',
                         'ifdm_geral', 'abstencao_var', 'precocidade']].dropna().copy()

        if len(df_win) > 20:
            # Winsorizar delta_vantagem no 99º percentil (top 1%)
            p99 = df_win['delta_vantagem'].quantile(0.99)
            p01 = df_win['delta_vantagem'].quantile(0.01)
            df_win['delta_vantagem_w'] = df_win['delta_vantagem'].clip(lower=p01, upper=p99)

            n_winsorized = (df_win['delta_vantagem'] != df_win['delta_vantagem_w']).sum()

            X_win = df_win[['NEC', 'gestao_index', 'ln_pop', 'ifdm_geral',
                           'abstencao_var', 'precocidade']]
            X_win = sm.add_constant(X_win)
            y_win = df_win['delta_vantagem_w']

            mod_win = sm.OLS(y_win, X_win).fit(cov_type='HC1')
            sens_winsor = {
                'status': 'executado',
                'N': len(df_win),
                'N_winsorized': int(n_winsorized),
                'percentis': {'p01': float(p01), 'p99': float(p99)},
                'coef': mod_win.params.to_dict(),
                'pvalues': mod_win.pvalues.to_dict(),
                'AIC': float(mod_win.aic),
                'nota': f'{n_winsorized} obs winsorized (top/bottom 1%)'
            }

    save_artifact('Sens_Winsor.json', json_obj=sens_winsor)

    # === 4.6.7: Comparação de inferência (cluster vs Mixed) ===
    logger.info("  - Sens 7/7: Comparação inferência")
    sens_infer = {'status': 'não executado'}

    if 'delta_vantagem' in df_work.columns and 'UF' in df_work.columns:
        df_inf = df_work[['delta_vantagem', 'NEC', 'gestao_index', 'ln_pop',
                         'ifdm_geral', 'abstencao_var', 'precocidade', 'UF']].dropna()

        if len(df_inf) > 20:
            X_inf = df_inf[['NEC', 'gestao_index', 'ln_pop', 'ifdm_geral',
                           'abstencao_var', 'precocidade']]
            X_inf = sm.add_constant(X_inf)
            y_inf = df_inf['delta_vantagem']

            # OLS com cluster-robust SE
            mod_cluster = sm.OLS(y_inf, X_inf).fit(cov_type='cluster',
                                                    cov_kwds={'groups': df_inf['UF']})

            # Mixed com intercepto aleatório
            md_mixed = MixedLM(y_inf, X_inf, groups=df_inf['UF'])
            mdf_mixed = md_mixed.fit(reml=False)

            sens_infer = {
                'status': 'executado',
                'N': len(df_inf),
                'cluster_SE': {
                    'coef': mod_cluster.params.to_dict(),
                    'se': mod_cluster.bse.to_dict(),
                    'pvalues': mod_cluster.pvalues.to_dict()
                },
                'mixed_SE': {
                    'coef': mdf_mixed.params.to_dict(),
                    'se': mdf_mixed.bse.to_dict(),
                    'pvalues': mdf_mixed.pvalues.to_dict(),
                    'var_RE': float(mdf_mixed.cov_re.iloc[0, 0]),
                    'converged': mdf_mixed.converged
                },
                'comparacao_NEC': {
                    'coef_diff': float(mod_cluster.params['NEC'] - mdf_mixed.params['NEC']),
                    'se_ratio': float(mod_cluster.bse['NEC'] / mdf_mixed.bse['NEC'])
                },
                'nota': 'Cluster SE vs Mixed SE - razões próximas de 1.0 indicam concordância'
            }

    save_artifact('Sens_Inferencia.json', json_obj=sens_infer)

    # === Resumo consolidado ===
    sens_resumo = {
        'NEC_HHI': sens_nec_hhi.get('status', 'não executado'),
        'NEC_ncand': sens_ncand.get('status', 'não executado'),
        'FormaPop': sens_formapop.get('status', 'não executado'),
        'SemCapitais': sens_semcap.get('status', 'não executado'),
        'LOUO': sens_louo.get('status', 'não executado'),
        'Winsor': sens_winsor.get('status', 'não executado'),
        'Inferencia': sens_infer.get('status', 'não executado'),
        'timestamp': str(datetime.datetime.now())
    }
    save_artifact('Sens_TODAS.json', json_obj=sens_resumo)
    logger.info(f"✓ Sensibilidades: {sum(1 for v in sens_resumo.values() if v == 'executado')} executadas")

    # ===== Manifesto e README =====
    with open(novos_dir / '00_manifest.json', 'w', encoding='utf-8') as f:
        json.dump(manifest, f, indent=2, ensure_ascii=False)

    readme_txt = f"""NOVAS ANÁLISES - README
========================

Gerado em: {datetime.datetime.now()}
CSV de entrada: {csv_path.name}
N amostra: {len(df)}

Arquivos gerados:
"""
    for arq in manifest['arquivos_gerados']:
        readme_txt += f"  - {arq}\n"

    readme_txt += """
Descrição dos arquivos:
- ICC.json: Intraclass Correlation Coefficients (clustering por UF)
- DeltaVantagem_*.json/*.png: Modelos OLS vs Mixed para ΔVantagem
- Reeleicao_*.json/*.csv/*.png: Modelos GLM Binomial para Reeleição
- NevoaIndex.csv: Índice de "névoa" padronizado com PCA
- Janela_30_60_90.csv: Distribuição temporal das medidas
- Sens_*.json: Análises de sensibilidade (7 análises)

Para mais detalhes, consulte run.log
"""

    with open(novos_dir / 'README_novos.txt', 'w', encoding='utf-8') as f:
        f.write(readme_txt)

    logger.info(f"=== NOVAS ANÁLISES CONCLUÍDAS ===")
    logger.info(f"Arquivos gerados: {len(manifest['arquivos_gerados'])}")
